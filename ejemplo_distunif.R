...
read.csv('~/Facultad/maestria/tesis/modulo_1_analitico/ejemplo_grafo/ptos.csv') -> ej.pts
...
#   maxi <- max(dis)
#   ndis <- length(dis)
#   dis[1:ndis]  <- runif(ndis, mini, maxi)
...
#     vertex.label.color=0, vertex.label.cex=.7)
  ...
  d_max <- max(dists)
  ndist <- length(dists)
  mst   <- set.edge.attribute(mst, 'weight',
  #######################
    value=rnorm(ndist, (d_max + d_min) / 2, sd=.5))
  #######################
  dists <- get.edge.attribute(mst, 'weight')
  d_min <- min(dists)
  d_max <- max(dists)
...
#   d_  <- seq(d_i, d_f, , nd_)
  ...
  resultado <- list(genera=puntos, puntos=pts, dists=dis,
